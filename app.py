...
st.caption("Paste a YouTube link, set timestamps, and download your MP4 clip.")
        ...
        with st.status("Processing...", expanded=True) as status:
            try:
                with tempfile.TemporaryDirectory() as tmp:
                    ...
                    st.write(f"Clipping from {start_ts} to {end_ts}...")
                    run_ffmpeg_clip(src, start_ts, end_ts, clip_path)

                    st.success("Clip ready for download.")
